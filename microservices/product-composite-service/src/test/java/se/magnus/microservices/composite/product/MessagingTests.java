...
class MessagingTests {
  ...
  @Autowired
  private OutputDestination target; // 비동기 메시지 수신 테스트 헬퍼
  ...
  // 메시지가 다른 서비스로 전파되는걸 테스트 하는게 아니고 product-composite 내에서 메시지가 정상적으로 수신되는지를 테스트
  @Test
  ...
  // getMessages / getMessage 가 호출되면 eventQueue 에 있는 Message 는 제거된다
